...
struct RestContext {
    ...
    config: RestCatalogConfig,
    /// Endpoints advertised by the server, used to gate server-side scan
    /// planning. Empty when the server does not advertise an `endpoints` list.
...
impl RestCatalog {
    ...
    /// can delegate planning to the server. It carries the storage factory and
    /// base props so it can build a plan-scoped `FileIO` (per-prefix) from the
    /// credentials the server vends in the plan response.
