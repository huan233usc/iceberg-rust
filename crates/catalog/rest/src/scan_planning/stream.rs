...
fn build_scan_file_io(
    ...
    // Scope each vended credential to its location prefix (mirroring how
    // `RestCatalog::load_file_io` wires load-table credentials), so reads of a
    // plan's data files use the matching credential.
    let mut builder = FileIOBuilder::new(factory).with_props(ctx.base_props.clone());
    for cred in wire_creds {
        let mut prefixed = ctx.base_props.clone();
        prefixed.extend(cred.config);
        builder = builder.with_prefixed_props(cred.prefix, prefixed);
    }
    Some(builder.build())
}
