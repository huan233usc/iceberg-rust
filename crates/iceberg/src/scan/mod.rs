...
pub use planner::{ScanPlanRequest, ServerScanPlan};
pub use task::*;
...
pub struct TableScan {
    ...
    /// Optional catalog handle injected when the table was loaded. When present,
    /// [`plan_files`](Self::plan_files) delegates planning to
    /// [`Catalog::plan_table_scan`].
...
impl TableScan {
    ...
    pub async fn plan_files(&self) -> Result<FileScanTaskStream> {
        ...
        // Delegate to server-side scan planning when a catalog handle is
        // present. A `FeatureUnsupported` error means the catalog/server does
        // not support planning, so we fall back to native planning.
        if let Some(catalog) = self.catalog.as_ref() {
            ...
            match catalog.plan_table_scan(request).await {
                Ok(plan) => {
